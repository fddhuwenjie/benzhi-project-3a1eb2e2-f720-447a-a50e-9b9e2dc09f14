...
func (s *Service) Get(ctx context.Context, caseID string) (*Detail, error) {
	...
	events, chainOK := s.loadTimeline(ctx, caseID)
	return &Detail{Case: item, Timeline: audit.Project(events), ChainOK: chainOK}, nil
}

func (s *Service) loadTimeline(ctx context.Context, caseID string) ([]audit.Event, bool) {
	events, err := s.store.Events(ctx, caseID)
	if err != nil {
		return nil, false
	}
	return events, audit.Verify(events) == nil
}
