...
func (s *Service) Get(ctx context.Context, caseID string) (*Detail, error) {
	...
	if detail, ok := cachedDetail(caseID, item.Revision); ok {
		return detail, nil
	}
	...
	timeline := audit.Project(events)
	cacheDetail(caseID, item, timeline, chainOK)
	return &Detail{Case: item, Timeline: timeline, ChainOK: chainOK}, nil
}
