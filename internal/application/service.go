...
type Service struct {
	store      Store
	now        func() time.Time
	eventHeads map[string]string
}

func NewService(store Store) *Service {
	return &Service{store: store, now: time.Now, eventHeads: map[string]string{}}
}
...
func (s *Service) mutate(ctx context.Context, caseID, operation, eventType string, meta CommandMeta, roles []string, payload any, mutate func(*domain.RetirementCase, time.Time) error) (*MutationResult, error) {
	...
	event, err := audit.NewEvent(caseID, eventType, meta.ActorID, meta.RequestID, item.Revision, now, payload, s.eventHead(caseID, events))
	if err != nil {
		return nil, err
	}
	...
	s.eventHeads[caseID] = event.EventHash
	return &MutationResult{Case: domain.Clone(item)}, nil
}

func (s *Service) eventHead(caseID string, events []audit.Event) string {
	current := audit.Head(events)
	if cached, ok := s.eventHeads[caseID]; ok && cached == current {
		return cached
	}
	s.eventHeads[caseID] = current
	return current
}
